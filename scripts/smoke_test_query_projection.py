...
def main():
    ...
    # a 2-channel "feature map" already in the ViT's own frame, where channel 0 encodes
    # normalized column position and channel 1 encodes normalized row position, so sampling
    # at a known location has a known, checkable value.
    ...
    # center of the original image must land near the center of the ViT frame (~0.5, ~0.5),
    # regardless of the original image's non-square aspect ratio -- this is exactly the case
    # the old (buggy) code, which normalized by the original image size against the resized+
    # cropped patch grid, would get wrong.
    ...
    # monotonicity: moving right/down in the original image must move the sampled position
    # right/down in the ViT frame too -- catches axis swaps, sign flips, and wrong scale
    # factors, without pinning brittle exact edge values (the width crop makes exact edge
    # arithmetic easy to get subtly wrong in the test itself, not just the code under test).
